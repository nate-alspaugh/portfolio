...
- Strained engineering — lots of resources spent on bugs`},{type:`callout`,eyebrow:`Key Takeaway`,body:`Mixed bag of retention — why did some adopt it and love it while others failed to?`},{type:`section`,heading:`Discovery — Users`,body:`This clue showed our users falling into a couple of camps:`},{type:`paragraph`,body:`**Adopters** those who successfully began using it and found value in it.`},{type:`inline-text`,text:`What we needed to find out:`},{type:`list-items`,items:[`Identify force multipliers — anywhere we could pour gas on the fire?`,`Why were they able to adopt it? What exact value were they getting?`]},{type:`paragraph`,body:`**Inactive users** those who failed to incorporate it into their workflows — new or existing.`},{type:`inline-text`,text:`What we needed to find out:`},{type:`list-items`,items:[`New users: what barriers kept it out of their workflow?`,`Existing users: why didn't they return to using it?`]},{type:`feature`,heading:`Emerging insights — Adopters`},{type:`feature`,heading:`Emerging insights — Inactive`},{type:`section`,heading:`Prioritization`,body:`Based on what we found, we focused on the "Note Creation" flow. To get the biggest bang for our buck, this area had the most surface that touched both optimization and adoption issues.`},{type:`section`,heading:`Ideation — Brainstorming`,body:`HMW: Make it easy for reps to quickly create a voice note?`,items:[`Leverage AI to handle the metadata (label their notes)`,`Unified flow — no up-front branching between note types (type vs voice)`,`Slash any underused functionality`]},{type:`feature`,caption:`Before — existing voice note flow`},{type:`feature`,caption:`After — new unified note creation flow`},{type:`section`,heading:`Demo Feedback`,body:`We showcased the flow for impression testing with some adopters and inactive users. Here are the insights we initially got:`},{type:`feature`,caption:`Demo feedback highlights from impression testing`},{type:`section`,heading:`Conclusion — Takeaways`,items:[`While I wasn't with the company at release, I stayed connected with Andrew. The launch was well received based on early feedback.`,`Dealing with constraints. We couldn't leverage automation for everything we'd like, so making the manual path easier was the next best thing: "When you can't remove a step, make it almost free."`,`Contextual inquiry is a game changer. Having the full context of a rep's day let us identify that through-line a bit better.`]}]}],ra={"particle-finance-research-canvas":{heroVideo:Ui,sections:{4:{video:Wi},5:{video:Gi},6:{images:[{src:Ki,alt:`Company ask question trigger`},{src:qi,alt:`Company ask question being asked`},{src:Ji,alt:`Company ask question response`}]},7:{image:Yi},8:{image:Xi},10:{image:ea},12:{image:Zi},13:{image:Qi},14:{image:$i},16:{image:ta}}},"particle-component-forge-skill":{heroImage:Hi},"voze-mobile-app-redesign":{heroImage:Li,sections:{11:{image:zi},12:{image:Bi},15:{video:Ri},18:{image:Vi}}}},ia=na.map(e=>{let{sections:t,...n}=ra[e.slug]??{},r=e.sections?.map((e,n)=>({...e,...t?.[n]??{}}));return{...e,...n,...r?{sections:r}:{}}});function aa(e){return ia.find(t=>t.slug===e)??null}var oa=new Set;function sa(e=window.location.pathname){let t=e.match(/^\/work\/([^/]+)\/?$/);return t?{name:`case-study`,slug:t[1]}:{name:`home`}}function ca(e){window.history.pushState({},``,e),ua()}function la(e){return oa.add(e),()=>oa.delete(e)}function ua(){let e=sa();oa.forEach(t=>t(e))}window.addEventListener(`popstate`,ua);var da=document.createElement(`template`);da.innerHTML=`
  <style>
    :host {
      display: block;
      font-family: var(--font-geist-mono);
...
`,ha=document.createElement(`template`);ha.innerHTML=`
  <style>
    :host {
      display: flex;
      flex-direction: column;
  ...
  <div class="rows"></div>
`;var ga=class extends HTMLElement{constructor(){super(),this.attachShadow({mode:`open`}),this.shadowRoot.appendChild(ha.content.cloneNode(!0))}connectedCallback(){let e=this.shadowRoot.querySelector(`.rows`);e.innerHTML=pa.map(e=>`
      <a class="row" href="${e.href}" target="_blank" rel="noopener noreferrer">
        <span class="row__label">${e.label}</span>
        ${ma}
      </a>
    `).join(``)}};customElements.define(`contact-links`,ga);function $(e){return String(e??``).replace(/&/g,`&amp;`).replace(/</g,`&lt;`).replace(/>/g,`&gt;`).replace(/"/g,`&quot;`)}function _a(e){return e.replace(/\*\*(.*?)\*\*/g,`<span class="bold-text">$1</span>`)}function va(e){return!e||!e.length?``:`<ul class="cs-section-list">${e.map(e=>`<li>${$(e)}</li>`).join(``)}</ul>`}function ya(e){switch(e.type){case`hero-placeholder`:return`<div class="cs-hero-placeholder">${$(e.label??``)}</div>`;case`section`:return`
      ...
      `;case`paragraph`:return`
        <div class="cs-paragraph">
          <p class="cs-paragraph-body">${_a(e.body).replace(/\n/g,`<br>`)}</p>
          ${va(e.items)}
        </div>
      `;case`inline-text`:return`<p class="inlineSubheading"><em><strong>${$(e.text)}</strong></em></p>`;case`list-items`:return va(e.items);case`callout`:return`
      ...
      `;case`image`:{let t=e.aspectRatio?` style="aspect-ratio: ${e.aspectRatio};"`:``;return e.image?`<img class="cs-bare-image" src="${e.image}" alt="${$(e.label??``)}"${t} />`:``}case`feature`:{let t=e.aspectRatio?` style="aspect-ratio: ${e.aspectRatio};"`:``,n=``;return e.video?n=`<video class="cs-feature-image" autoplay loop muted playsinline preload="auto"${t}><source src="${e.video}" type="video/mp4" /></video>`:e.image&&(n=`<img class="cs-feature-image" src="${e.image}" alt="${$(e.caption??e.label??``)}"${t} />`),`
        ${e.heading?`<h3 class="cs-feature-heading">${$(e.heading)}</h3>`:``}
      ...
      `}default:return``}}function ba(e){if(!e)return`<div class="cs-not-found">Project not found — <a href="/" style="color: #99cc00">back home</a></div>`;let t=e.heroVideo?`<video class="cs-hero-image" autoplay loop muted playsinline preload="auto"><source src="${e.heroVideo}" type="video/mp4" /></video>`:e.heroImage?`<img class="cs-hero-image" src="${e.heroImage}" alt="${$(e.title)}" />`:``,n=e.sections?`<div class="cs-sections">${e.sections.map(ya).join(``)}</div>`:`<div class="cs-coming-soon">Coming soon</div>`,r=e.intro?`<p class="cs-intro">${$(e.intro)}</p>`:``;return`
    <div class="cs-eyebrow">Case study · ${$(e.year??``)}</div>
    <h1 class="cs-title">${$(e.title)}</h1>
    ${r}
    ${t}
    ${n}
  `}var xa=class extends HTMLElement{static get observedAttributes(){return[`slug`]}constructor(){super(),this._onBack=this._onBack.bind(this)}connectedCallback(){this._render()}disconnectedCallback(){this._teardown()}attributeChangedCallback(e,t,n){!this.isConnected||t===n||this._render()}_render(){let e=aa(this.getAttribute(`slug`)??``);e?.accent&&this.style.setProperty(`--accent`,e.accent),this.innerHTML=`
      <div class="cs-wrap">
        ...
        <div class="cs-content">${ba(e)}</div>
      </div>
    `,this._wire()}_wire(){this._teardown();let e=this.querySelector(`.cs-back`);e&&e.addEventListener(`click`,this._onBack),this._setupVideoPlayback(),this._setupCarousels()}_teardown(){let e=this.querySelector(`.cs-back`);e&&e.removeEventListener(`click`,this._onBack),this._teardownVideoPlayback?.(),this._teardownCarousels?.()}_onBack(){ca(`/`)}_setupVideoPlayback(){this._teardownVideoPlayback?.();let e=Array.from(this.querySelectorAll(`video`));if(!e.length)return;let t=()=>{let t=window.innerHeight;for(let n of e){let e=n.getBoundingClientRect(),r=e.top+e.height/2;r>0&&r<t?n.paused&&n.play().catch(()=>{}):n.paused||n.pause()}},n=0,r=()=>{n||=requestAnimationFrame(()=>{n=0,t()})};window.addEventListener(`scroll`,r,{passive:!0}),window.addEventListener(`resize`,r),e.forEach(e=>e.addEventListener(`loadedmetadata`,t)),t(),this._teardownVideoPlayback=()=>{window.removeEventListener(`scroll`,r),window.removeEventListener(`resize`,r),n&&cancelAnimationFrame(n),this._teardownVideoPlayback=null}}_setupCarousels(){this._teardownCarousels?.();let e=Array.from(this.querySelectorAll(`.cs-carousel`));if(!e.length)return;let t=e.map(e=>{let t=e.querySelector(`.cs-carousel-track`),n=e.querySelector(`.cs-carousel-arrow.prev`),r=e.querySelector(`.cs-carousel-arrow.next`),i=Array.from(e.querySelectorAll(`.cs-carousel-dot`)),a=Number(e.dataset.count)||0,o=0,s=()=>{t.style.transform=`translateX(-${o*100}%)`,i.forEach((e,t)=>e.classList.toggle(`active`,t===o)),n.disabled=o===0,r.disabled=o>=a-1},c=e=>{o=Math.max(0,Math.min(a-1,e)),s()},l=()=>c(o-1),u=()=>c(o+1),d=e=>{let t=Number(e.currentTarget.dataset.index);Number.isNaN(t)||c(t)};return n.addEventListener(`click`,l),r.addEventListener(`click`,u),i.forEach(e=>e.addEventListener(`click`,d)),s(),()=>{n.removeEventListener(`click`,l),r.removeEventListener(`click`,u),i.forEach(e=>e.removeEventListener(`click`,d))}});this._teardownCarousels=()=>{t.forEach(e=>e()),this._teardownCarousels=null}}};customElements.define(`case-study-page`,xa);var Sa=`https://nathanalspaugh.com`,Ca=`Nathan Alspaugh | AI-Native Sr. Product Designer`,wa=`Senior product designer leveraging AI to ship ambitious work. Case studies, contact, and selected projects.`,Ta=`${Sa}/og-image.png`;function Ea(e,t){return e.name===`case-study`&&t?{title:`Nathan Alspaugh | ${t.title}`,description:t.description??`Senior product designer leveraging AI to ship ambitious work. Case studies, contact, and selected projects.`,url:`${Sa}/work/${t.slug}`,ogImage:Ta}:{title:Ca,description:wa,url:Sa+`/`,ogImage:Ta}}function Da(e,t,n){let r=document.head.querySelector(e);if(!r){r=document.createElement(`meta`);let[t,n]=e.replace(/^meta\[|\]$/g,``).split(`=`);r.setAttribute(t,n.replace(/"/g,``)),document.head.appendChild(r)}r.setAttribute(t,n)}function Oa(e,t){let n=document.head.querySelector(`link[rel="${e}"]`);n||(n=document.createElement(`link`),n.setAttribute(`rel`,e),document.head.appendChild(n)),n.setAttribute(`href`,t)}function ka({title:e,description:t,url:n,ogImage:r}){document.title=e,Da(`meta[name="description"]`,`content`,t),Da(`meta[property="og:title"]`,`content`,e),Da(`meta[property="og:description"]`,`content`,t),Da(`meta[property="og:url"]`,`content`,n),Da(`meta[property="og:image"]`,`content`,r),Da(`meta[property="og:type"]`,`content`,`website`),Da(`meta[name="twitter:card"]`,`content`,`summary_large_image`),Da(`meta[name="twitter:title"]`,`content`,e),Da(`meta[name="twitter:description"]`,`content`,t),Da(`meta[name="twitter:image"]`,`content`,r),Oa(`canonical`,n)}var Aa=()=>{window.va||(window.va=function(...e){window.vaq||(window.vaq=[]),window.vaq.push(e)})},ja=`@vercel/analytics`,Ma=`2.0.1`;function Na(){return typeof window<`u`}function Pa(){try{let e=`production`;if(e===`development`||e===`test`)return`development`}catch{}return`production`}function Fa(e=`auto`){if(e===`auto`){window.vam=Pa();return}window.vam=e}function Ia(){return(Na()?window.vam:Pa())||`production`}function La(){return Ia()===`development`}function Ra(e){return e.scriptSrc?Ba(e.scriptSrc):La()?`https://va.vercel-scripts.com/v1/script.debug.js`:e.basePath?Ba(`${e.basePath}/insights/script.js`):`/_vercel/insights/script.js`}function za(e,t){let n=e;if(t)try{n={...JSON.parse(t)?.analytics,...e}}catch{}Fa(n.mode);let r={sdkn:ja+(n.framework?`/${n.framework}`:``),sdkv:Ma};return n.disableAutoTrack&&(r.disableAutoTrack=`1`),n.viewEndpoint&&(r.viewEndpoint=Ba(n.viewEndpoint)),n.eventEndpoint&&(r.eventEndpoint=Ba(n.eventEndpoint)),n.sessionEndpoint&&(r.sessionEndpoint=Ba(n.sessionEndpoint)),La()&&n.debug===!1&&(r.debug=`false`),n.dsn&&(r.dsn=n.dsn),n.endpoint?r.endpoint=n.endpoint:n.basePath&&(r.endpoint=Ba(`${n.basePath}/insights`)),{beforeSend:n.beforeSend,src:Ra(n),dataset:r}}function Ba(e){return e.startsWith(`http://`)||e.startsWith(`https://`)||e.startsWith(`/`)?e:`/${e}`}function Va(e={debug:!0},t){var n;if(!Na())return;let{beforeSend:r,src:i,dataset:a}=za(e,t);if(Aa(),r&&((n=window.va)==null||n.call(window,`beforeSend`,r)),document.head.querySelector(`script[src*="${i}"]`))return;let o=document.createElement(`script`);o.src=i;for(let[e,t]of Object.entries(a))o.dataset[e]=t;o.defer=!0,o.onerror=()=>{let e=La()?`Please check if any ad blockers are enabled and try again.`:`Be sure to enable Web Analytics for your project and deploy again. See https://vercel.com/docs/analytics/quickstart for more information.`;console.log(`[Vercel Web Analytics] Failed to load script from ${i}. ${e}`)},document.head.appendChild(o)}Va();var Ha=document.querySelector(`#app`);function Ua(){Ha.querySelector(`.hero`)||(Ha.innerHTML=`
    <section class="hero">
  ...
  `)}function Wa(e){Ha.querySelector(`case-study-page[slug="${CSS.escape(e)}"]`)||(Ha.innerHTML=`<case-study-page slug="${e}"></case-study-page>`)}function Ga(e){ka(Ea(e,e.name===`case-study`?aa(e.slug):null)),e.name===`case-study`?Wa(e.slug):Ua()}Ga(sa()),la(Ga);