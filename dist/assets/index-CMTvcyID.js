...
`;var fi=class extends HTMLElement{constructor(){super(),this.attachShadow({mode:`open`}),this.shadowRoot.appendChild(di.content.cloneNode(!0)),this._card=null,this._onMouseMove=this._onMouseMove.bind(this),this._onMouseLeave=this._onMouseLeave.bind(this)}connectedCallback(){let e=this.shadowRoot.querySelector(`.photo`);e.src=ui,this._card=this.shadowRoot.querySelector(`.key-card`),this._card.addEventListener(`mousemove`,this._onMouseMove),this._card.addEventListener(`mouseleave`,this._onMouseLeave),this._animate()}disconnectedCallback(){this._card&&(this._card.removeEventListener(`mousemove`,this._onMouseMove),this._card.removeEventListener(`mouseleave`,this._onMouseLeave))}_onMouseMove(e){let t=this._card.getBoundingClientRect(),n=(e.clientX-t.left)/t.width*100,r=(e.clientY-t.top)/t.height*100;this._card.style.setProperty(`--mx`,`${n}%`),this._card.style.setProperty(`--my`,`${r}%`);let i=(n-50)*.01875,a=(r-50)*-.0125;li.to(this._card,{rotateX:a,rotateY:i,duration:.3,ease:`power2.out`,overwrite:`auto`})}_onMouseLeave(){this._card.style.setProperty(`--mx`,`50%`),this._card.style.setProperty(`--my`,`30%`),li.to(this._card,{rotateX:0,rotateY:0,duration:.5,ease:`power2.out`,overwrite:`auto`})}_animate(){setTimeout(()=>{this._floatTween=li.to(this._card,{y:-8,duration:2.5,ease:`sine.inOut`,yoyo:!0,repeat:-1})},1100)}};customElements.define(`key-card`,fi);var pi=document.createElement(`template`);pi.innerHTML=`
  <style>
    :host {
      display: block;
      position: fixed;
...
`;var Ji=class extends HTMLElement{constructor(){super(),this.attachShadow({mode:`open`}),this.shadowRoot.appendChild(qi.content.cloneNode(!0))}connectedCallback(){let e=this.shadowRoot.querySelector(`.rows`);e.innerHTML=Gi.map(e=>`
    ...
    `).join(``)}};customElements.define(`contact-links`,Ji);function $(e){return String(e??``).replace(/&/g,`&amp;`).replace(/</g,`&lt;`).replace(/>/g,`&gt;`).replace(/"/g,`&quot;`)}function Yi(e){return!e||!e.length?``:`<ul class="cs-section-list">${e.map(e=>`<li>${$(e)}</li>`).join(``)}</ul>`}function Xi(e){switch(e.type){case`hero-placeholder`:return`<div class="cs-hero-placeholder">${$(e.label??``)}</div>`;case`section`:return`
      ...
      `}default:return``}}function Zi(e){if(!e)return`<div class="cs-not-found">Project not found — <a href="/" style="color: #99cc00">back home</a></div>`;let t=e.heroVideo?`<video class="cs-hero-image" autoplay loop muted playsinline preload="auto"><source src="${e.heroVideo}" type="video/mp4" /></video>`:e.heroImage?`<img class="cs-hero-image" src="${e.heroImage}" alt="${$(e.title)}" />`:``,n=e.sections?`<div class="cs-sections">${e.sections.map(Xi).join(``)}</div>`:`<div class="cs-coming-soon">Coming soon</div>`,r=e.intro?`<p class="cs-intro">${$(e.intro)}</p>`:``;return`
    <div class="cs-eyebrow">Case study · ${$(e.year??``)}</div>
    <h1 class="cs-title">${$(e.title)}</h1>
    ${r}
    ${t}
    ${n}
  `}var Qi=class extends HTMLElement{static get observedAttributes(){return[`slug`]}constructor(){super(),this._onBack=this._onBack.bind(this)}connectedCallback(){this._render()}disconnectedCallback(){this._teardown()}attributeChangedCallback(e,t,n){!this.isConnected||t===n||this._render()}_render(){let e=Li(this.getAttribute(`slug`)??``);e?.accent&&this.style.setProperty(`--accent`,e.accent),this.innerHTML=`
      <div class="cs-wrap">
        ...
        <div class="cs-content">${Zi(e)}</div>
      </div>
    `,this._wire()}_wire(){this._teardown();let e=this.querySelector(`.cs-back`);e&&e.addEventListener(`click`,this._onBack),this._setupVideoPlayback(),this._setupCarousels()}_teardown(){let e=this.querySelector(`.cs-back`);e&&e.removeEventListener(`click`,this._onBack),this._teardownVideoPlayback?.(),this._teardownCarousels?.()}_onBack(){Bi(`/`)}_setupVideoPlayback(){this._teardownVideoPlayback?.();let e=Array.from(this.querySelectorAll(`video`));if(!e.length)return;let t=()=>{let t=window.innerHeight;for(let n of e){let e=n.getBoundingClientRect(),r=e.top+e.height/2;r>0&&r<t?n.paused&&n.play().catch(()=>{}):n.paused||n.pause()}},n=0,r=()=>{n||=requestAnimationFrame(()=>{n=0,t()})};window.addEventListener(`scroll`,r,{passive:!0}),window.addEventListener(`resize`,r),e.forEach(e=>e.addEventListener(`loadedmetadata`,t)),t(),this._teardownVideoPlayback=()=>{window.removeEventListener(`scroll`,r),window.removeEventListener(`resize`,r),n&&cancelAnimationFrame(n),this._teardownVideoPlayback=null}}_setupCarousels(){this._teardownCarousels?.();let e=Array.from(this.querySelectorAll(`.cs-carousel`));if(!e.length)return;let t=e.map(e=>{let t=e.querySelector(`.cs-carousel-track`),n=e.querySelector(`.cs-carousel-arrow.prev`),r=e.querySelector(`.cs-carousel-arrow.next`),i=Array.from(e.querySelectorAll(`.cs-carousel-dot`)),a=Number(e.dataset.count)||0,o=0,s=()=>{t.style.transform=`translateX(-${o*100}%)`,i.forEach((e,t)=>e.classList.toggle(`active`,t===o)),n.disabled=o===0,r.disabled=o>=a-1},c=e=>{o=Math.max(0,Math.min(a-1,e)),s()},l=()=>c(o-1),u=()=>c(o+1),d=e=>{let t=Number(e.currentTarget.dataset.index);Number.isNaN(t)||c(t)};return n.addEventListener(`click`,l),r.addEventListener(`click`,u),i.forEach(e=>e.addEventListener(`click`,d)),s(),()=>{n.removeEventListener(`click`,l),r.removeEventListener(`click`,u),i.forEach(e=>e.removeEventListener(`click`,d))}});this._teardownCarousels=()=>{t.forEach(e=>e()),this._teardownCarousels=null}}};customElements.define(`case-study-page`,Qi);var $i=`https://nathanalspaugh.com`,ea=`Nathan Alspaugh | AI-Native Sr. Product Designer`,ta=`Senior product designer leveraging AI to ship ambitious work. Case studies, contact, and selected projects.`,na=`${$i}/og-image.png`;function ra(e,t){return e.name===`case-study`&&t?{title:`Nathan Alspaugh | ${t.title}`,description:t.description??`Senior product designer leveraging AI to ship ambitious work. Case studies, contact, and selected projects.`,url:`${$i}/work/${t.slug}`,ogImage:na}:{title:ea,description:ta,url:$i+`/`,ogImage:na}}function ia(e,t,n){let r=document.head.querySelector(e);if(!r){r=document.createElement(`meta`);let[t,n]=e.replace(/^meta\[|\]$/g,``).split(`=`);r.setAttribute(t,n.replace(/"/g,``)),document.head.appendChild(r)}r.setAttribute(t,n)}function aa(e,t){let n=document.head.querySelector(`link[rel="${e}"]`);n||(n=document.createElement(`link`),n.setAttribute(`rel`,e),document.head.appendChild(n)),n.setAttribute(`href`,t)}function oa({title:e,description:t,url:n,ogImage:r}){document.title=e,ia(`meta[name="description"]`,`content`,t),ia(`meta[property="og:title"]`,`content`,e),ia(`meta[property="og:description"]`,`content`,t),ia(`meta[property="og:url"]`,`content`,n),ia(`meta[property="og:image"]`,`content`,r),ia(`meta[property="og:type"]`,`content`,`website`),ia(`meta[name="twitter:card"]`,`content`,`summary_large_image`),ia(`meta[name="twitter:title"]`,`content`,e),ia(`meta[name="twitter:description"]`,`content`,t),ia(`meta[name="twitter:image"]`,`content`,r),aa(`canonical`,n)}var sa=document.querySelector(`#app`);function ca(){sa.querySelector(`.hero`)||(sa.innerHTML=`
    <section class="hero">
  ...
  `)}function la(e){sa.querySelector(`case-study-page[slug="${CSS.escape(e)}"]`)||(sa.innerHTML=`<case-study-page slug="${e}"></case-study-page>`)}function ua(e){oa(ra(e,e.name===`case-study`?Li(e.slug):null)),e.name===`case-study`?la(e.slug):ca()}ua(zi()),Vi(ua);