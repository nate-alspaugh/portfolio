...
`,Ai=document.createElement(`template`);Ai.innerHTML=`
  <style>
    :host {
      display: flex;
      flex-direction: column;
  ...
  <div class="rows"></div>
`;var ji=class extends HTMLElement{constructor(){super(),this.attachShadow({mode:`open`}),this.shadowRoot.appendChild(Ai.content.cloneNode(!0))}connectedCallback(){let e=this.shadowRoot.querySelector(`.rows`);e.innerHTML=Oi.map(e=>`
      <a class="row" href="${e.href}" target="_blank" rel="noopener noreferrer">
        <span class="row__label">${e.label}</span>
        ${ki}
      </a>
    `).join(``)}};customElements.define(`contact-links`,ji);var Mi=document.createElement(`template`);Mi.innerHTML=`
  <style>
    :host {
      display: block;
      min-height: 100vh;
...
`;function $(e){return String(e).replace(/&/g,`&amp;`).replace(/</g,`&lt;`).replace(/>/g,`&gt;`)}function Ni(e){return!e||!e.length?``:`<ul class="section-list">${e.map(e=>`<li>${$(e)}</li>`).join(``)}</ul>`}function Pi(e){switch(e.type){case`hero-placeholder`:return`<div class="hero-placeholder">${$(e.label??``)}</div>`;case`section`:return`
  ...
  `}function Bi(e){Ii.innerHTML=`<case-study-page slug="${e}"></case-study-page>`}function Vi(e){Ri(e),e.name===`case-study`?Bi(e.slug):zi()}Vi(Si()),wi(Vi);