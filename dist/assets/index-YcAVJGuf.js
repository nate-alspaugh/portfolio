...
`;var fa=class extends HTMLElement{constructor(){super(),this.attachShadow({mode:`open`}),this.shadowRoot.appendChild(da.content.cloneNode(!0))}connectedCallback(){let e=this.shadowRoot.querySelector(`.rows`);e.innerHTML=la.map(e=>`
    ...
    `).join(``)}};customElements.define(`contact-links`,fa);function $(e){return String(e??``).replace(/&/g,`&amp;`).replace(/</g,`&lt;`).replace(/>/g,`&gt;`).replace(/"/g,`&quot;`)}function pa(e){return e.replace(/\*\*(.*?)\*\*/g,`<span class="bold-text">$1</span>`)}function ma(e){return!e||!e.length?``:`<ul class="cs-section-list">${e.map(e=>`<li>${$(e)}</li>`).join(``)}</ul>`}function ha(e){switch(e.type){case`hero-placeholder`:return`<div class="cs-hero-placeholder">${$(e.label??``)}</div>`;case`section`:return`
      ...
      `;case`paragraph`:return`
        <p class="cs-paragraph-body">${pa(e.body).replace(/\n/g,`<br>`)}</p>
        ${ma(e.items)}
      `;case`inline-text`:return`<p class="inlineSubheading"><em><strong>${$(e.text)}</strong></em></p>`;case`list-items`:return ma(e.items);case`callout`:return`
      ...
      `}default:return``}}function ga(e){if(!e)return`<div class="cs-not-found">Project not found — <a href="/" style="color: #99cc00">back home</a></div>`;let t=e.heroVideo?`<video class="cs-hero-image" autoplay loop muted playsinline preload="auto"><source src="${e.heroVideo}" type="video/mp4" /></video>`:e.heroImage?`<img class="cs-hero-image" src="${e.heroImage}" alt="${$(e.title)}" />`:``,n=e.sections?`<div class="cs-sections">${e.sections.map(ha).join(``)}</div>`:`<div class="cs-coming-soon">Coming soon</div>`,r=e.intro?`<p class="cs-intro">${$(e.intro)}</p>`:``;return`
    <div class="cs-eyebrow">Case study · ${$(e.year??``)}</div>
    <h1 class="cs-title">${$(e.title)}</h1>
    ${r}
    ${t}
    ${n}
  `}var _a=class extends HTMLElement{static get observedAttributes(){return[`slug`]}constructor(){super(),this._onBack=this._onBack.bind(this)}connectedCallback(){this._render()}disconnectedCallback(){this._teardown()}attributeChangedCallback(e,t,n){!this.isConnected||t===n||this._render()}_render(){let e=ta(this.getAttribute(`slug`)??``);e?.accent&&this.style.setProperty(`--accent`,e.accent),this.innerHTML=`
      <div class="cs-wrap">
        ...
        <div class="cs-content">${ga(e)}</div>
      </div>
    `,this._wire()}_wire(){this._teardown();let e=this.querySelector(`.cs-back`);e&&e.addEventListener(`click`,this._onBack),this._setupVideoPlayback(),this._setupCarousels()}_teardown(){let e=this.querySelector(`.cs-back`);e&&e.removeEventListener(`click`,this._onBack),this._teardownVideoPlayback?.(),this._teardownCarousels?.()}_onBack(){ia(`/`)}_setupVideoPlayback(){this._teardownVideoPlayback?.();let e=Array.from(this.querySelectorAll(`video`));if(!e.length)return;let t=()=>{let t=window.innerHeight;for(let n of e){let e=n.getBoundingClientRect(),r=e.top+e.height/2;r>0&&r<t?n.paused&&n.play().catch(()=>{}):n.paused||n.pause()}},n=0,r=()=>{n||=requestAnimationFrame(()=>{n=0,t()})};window.addEventListener(`scroll`,r,{passive:!0}),window.addEventListener(`resize`,r),e.forEach(e=>e.addEventListener(`loadedmetadata`,t)),t(),this._teardownVideoPlayback=()=>{window.removeEventListener(`scroll`,r),window.removeEventListener(`resize`,r),n&&cancelAnimationFrame(n),this._teardownVideoPlayback=null}}_setupCarousels(){this._teardownCarousels?.();let e=Array.from(this.querySelectorAll(`.cs-carousel`));if(!e.length)return;let t=e.map(e=>{let t=e.querySelector(`.cs-carousel-track`),n=e.querySelector(`.cs-carousel-arrow.prev`),r=e.querySelector(`.cs-carousel-arrow.next`),i=Array.from(e.querySelectorAll(`.cs-carousel-dot`)),a=Number(e.dataset.count)||0,o=0,s=()=>{t.style.transform=`translateX(-${o*100}%)`,i.forEach((e,t)=>e.classList.toggle(`active`,t===o)),n.disabled=o===0,r.disabled=o>=a-1},c=e=>{o=Math.max(0,Math.min(a-1,e)),s()},l=()=>c(o-1),u=()=>c(o+1),d=e=>{let t=Number(e.currentTarget.dataset.index);Number.isNaN(t)||c(t)};return n.addEventListener(`click`,l),r.addEventListener(`click`,u),i.forEach(e=>e.addEventListener(`click`,d)),s(),()=>{n.removeEventListener(`click`,l),r.removeEventListener(`click`,u),i.forEach(e=>e.removeEventListener(`click`,d))}});this._teardownCarousels=()=>{t.forEach(e=>e()),this._teardownCarousels=null}}};customElements.define(`case-study-page`,_a);var va=`https://nathanalspaugh.com`,ya=`Nathan Alspaugh | AI-Native Sr. Product Designer`,ba=`Senior product designer leveraging AI to ship ambitious work. Case studies, contact, and selected projects.`,xa=`${va}/og-image.png`;function Sa(e,t){return e.name===`case-study`&&t?{title:`Nathan Alspaugh | ${t.title}`,description:t.description??`Senior product designer leveraging AI to ship ambitious work. Case studies, contact, and selected projects.`,url:`${va}/work/${t.slug}`,ogImage:xa}:{title:ya,description:ba,url:va+`/`,ogImage:xa}}function Ca(e,t,n){let r=document.head.querySelector(e);if(!r){r=document.createElement(`meta`);let[t,n]=e.replace(/^meta\[|\]$/g,``).split(`=`);r.setAttribute(t,n.replace(/"/g,``)),document.head.appendChild(r)}r.setAttribute(t,n)}function wa(e,t){let n=document.head.querySelector(`link[rel="${e}"]`);n||(n=document.createElement(`link`),n.setAttribute(`rel`,e),document.head.appendChild(n)),n.setAttribute(`href`,t)}function Ta({title:e,description:t,url:n,ogImage:r}){document.title=e,Ca(`meta[name="description"]`,`content`,t),Ca(`meta[property="og:title"]`,`content`,e),Ca(`meta[property="og:description"]`,`content`,t),Ca(`meta[property="og:url"]`,`content`,n),Ca(`meta[property="og:image"]`,`content`,r),Ca(`meta[property="og:type"]`,`content`,`website`),Ca(`meta[name="twitter:card"]`,`content`,`summary_large_image`),Ca(`meta[name="twitter:title"]`,`content`,e),Ca(`meta[name="twitter:description"]`,`content`,t),Ca(`meta[name="twitter:image"]`,`content`,r),wa(`canonical`,n)}var Ea=()=>{window.va||(window.va=function(...e){window.vaq||(window.vaq=[]),window.vaq.push(e)})},Da=`@vercel/analytics`,Oa=`2.0.1`;function ka(){return typeof window<`u`}function Aa(){try{let e=`production`;if(e===`development`||e===`test`)return`development`}catch{}return`production`}function ja(e=`auto`){if(e===`auto`){window.vam=Aa();return}window.vam=e}function Ma(){return(ka()?window.vam:Aa())||`production`}function Na(){return Ma()===`development`}function Pa(e){return e.scriptSrc?Ia(e.scriptSrc):Na()?`https://va.vercel-scripts.com/v1/script.debug.js`:e.basePath?Ia(`${e.basePath}/insights/script.js`):`/_vercel/insights/script.js`}function Fa(e,t){let n=e;if(t)try{n={...JSON.parse(t)?.analytics,...e}}catch{}ja(n.mode);let r={sdkn:Da+(n.framework?`/${n.framework}`:``),sdkv:Oa};return n.disableAutoTrack&&(r.disableAutoTrack=`1`),n.viewEndpoint&&(r.viewEndpoint=Ia(n.viewEndpoint)),n.eventEndpoint&&(r.eventEndpoint=Ia(n.eventEndpoint)),n.sessionEndpoint&&(r.sessionEndpoint=Ia(n.sessionEndpoint)),Na()&&n.debug===!1&&(r.debug=`false`),n.dsn&&(r.dsn=n.dsn),n.endpoint?r.endpoint=n.endpoint:n.basePath&&(r.endpoint=Ia(`${n.basePath}/insights`)),{beforeSend:n.beforeSend,src:Pa(n),dataset:r}}function Ia(e){return e.startsWith(`http://`)||e.startsWith(`https://`)||e.startsWith(`/`)?e:`/${e}`}function La(e={debug:!0},t){var n;if(!ka())return;let{beforeSend:r,src:i,dataset:a}=Fa(e,t);if(Ea(),r&&((n=window.va)==null||n.call(window,`beforeSend`,r)),document.head.querySelector(`script[src*="${i}"]`))return;let o=document.createElement(`script`);o.src=i;for(let[e,t]of Object.entries(a))o.dataset[e]=t;o.defer=!0,o.onerror=()=>{let e=Na()?`Please check if any ad blockers are enabled and try again.`:`Be sure to enable Web Analytics for your project and deploy again. See https://vercel.com/docs/analytics/quickstart for more information.`;console.log(`[Vercel Web Analytics] Failed to load script from ${i}. ${e}`)},document.head.appendChild(o)}La();var Ra=document.querySelector(`#app`);function za(){Ra.querySelector(`.hero`)||(Ra.innerHTML=`
    <section class="hero">
  ...
  `)}function Ba(e){Ra.querySelector(`case-study-page[slug="${CSS.escape(e)}"]`)||(Ra.innerHTML=`<case-study-page slug="${e}"></case-study-page>`)}function Va(e){Ta(Sa(e,e.name===`case-study`?ta(e.slug):null)),e.name===`case-study`?Ba(e.slug):za()}Va(ra()),aa(Va);